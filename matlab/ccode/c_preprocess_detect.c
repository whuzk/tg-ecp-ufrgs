...
void removeBaseline()
{
    ...
    mean = 0.0;
    for (mwSize i = startIdx; i < startIdx + NUM_BL_SAMPLES; i++) {
        mean += noisb(i);
    }
    x[0] = 0.0;
    ...
    mean = 0.0;
    for (mwSize i = endIdx - NUM_BL_SAMPLES + 1; i <= endIdx; i++) {
        mean += noisb(i);
    }
    x[1] = endIdx - startIdx;
...
{
    bool qrsDetected, beatDetected, isArtifact;
